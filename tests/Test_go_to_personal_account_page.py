...
def test_go_to_personal_account_page():
    ...
    (TestLocators.Account_text)))

    assert driver.current_url == "https://stellarburgers.nomoreparties.site/account/profile" and driver.find_element(*TestLocators.Button_profile).text == "Профиль" and driver.find_element(*TestLocators.Button_history).text == "История заказов" and driver.find_element(*TestLocators.Button_exit).text == "Выход" and driver.find_element(*TestLocators.Account_text).text == "В этом разделе вы можете изменить свои персональные данные"
